from collections import Counter

from algorithms.models import lg_lottery_winner_number


def get_numbers(lottery_type_instance, modulo=3):
    past_draws = lg_lottery_winner_number.objects.filter(lottery_type=lottery_type_instance).values_list('lottery_type_number', flat=True)
    modulo_counter = Counter()

    for draw in past_draws:
        for number in draw:
            modulo_value = number % modulo
            modulo_counter[modulo_value] += 1

    common_modulo_values = [value for value, _ in modulo_counter.most_common()]
    predicted_numbers = set()
    for value in common_modulo_values:
        for number in range(value, lottery_type_instance.max_number + 1, modulo):
            predicted_numbers.add(number)
            if len(predicted_numbers) >= lottery_type_instance.pieces_of_draw_numbers:
                break

    return sorted(predicted_numbers)[:lottery_type_instance.pieces_of_draw_numbers]
