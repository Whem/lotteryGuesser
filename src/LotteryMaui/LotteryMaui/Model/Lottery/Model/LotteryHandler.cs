...
namespace LotteryMaui.Model.Lottery.Model
{
    ...
    public class LotteryHandler
    {
        ...
        public void LoadNeccessaryInformation(Enums.LotteryType lotteryType, int userId, bool isUseEarlierStatistic)
        {
        ...
        /// The link.
        /// </param>
        public async Task DownloadNumbersFromInternet(string link)
        {
            this.lotteryCollection = new List<LotteryModel>();
            using HttpClient client = new HttpClient();
            // WebClient class inherits IDisposable
            // Or you can get the file content without saving it
            string htmlCode = await client.GetStringAsync(link);
        ...
        {
            if (_dataContext.LotteryNumbers != null)
            {
                var getLotteryDrawing = _dataContext.LotteryNumbers
        ...
        public void UseEarlierWeekPercentageForNumbersDraw(Enums.TypesOfDrawn typesOfDrawn)
        {
            if (_dataContext.LotteryNumbers != null)
            {
                var getLotteryDrawing = _dataContext.LotteryNumbers.Where(x => x.WeekOfPull == this.lotteryCollection.Last().WeekOfLotteryDrawing).ToList();
                ...
                {
                    foreach (var saveNumber in getLotteryDrawing)
                    {
                        LotteryModel lm = new LotteryModel(lotteryRule);
